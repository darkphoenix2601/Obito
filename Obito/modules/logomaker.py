...
from Obito import OWNER_ID, SUPPORT_CHAT
from Obito.events import register
from Obito import telethn
...
async def lego(event):
 ...
 if event.sender_id == OWNER_ID:
  ...
  if not quew:
     ...
     return
 pesan = await event.reply('Making Logo. Please Wait.')
 try:
    ...
    fname = "Sophia.jpg"
    img.save(fname, "png")
    await telethn.send_file(event.chat_id, file=fname, caption = f"Made by @Obito2_0_bot")         
    await pesan.delete()
